...
func Tokenize(code string, commentChar string) []*Token {
	var tokens []*Token
	line := 1
	if len(code) == 0 {
		return tokens
	...
	for {
		...
		if c >= "0" && c <= "9" {
			...
			for {
				form += c
				if len(code) == 0 {
					tokens = append(tokens, &Token{Number, line, form})
					goto finished
				}
				c = code[:1]
				code = code[1:]
		...
		if c >= "A" && c <= "Z" || c >= "a" && c <= "z" {
			...
			for {
				form += c
				if len(code) == 0 {
					tokens = append(tokens, &Token{Identifier, line, form})
					goto finished
				}
				c = code[:1]
				code = code[1:]
		...
		if c == `"` {
			...
			for {
				if len(code) == 0 {
					tokens = append(tokens, &Token{String, line, form})
					goto finished
				}
				c = code[:1]
				code = code[1:]
		...
		tokens = append(tokens, &Token{Symbol, line, c})
		c = code[:1]
		code = code[1:]
	}
finished:
